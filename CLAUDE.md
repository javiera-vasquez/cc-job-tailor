...
**Key Features:**
- **Data-Driven Resume Generation**: Uses structured YAML data (`resume-data/resume.yaml`) containing comprehensive resume information
...
│   │   ├── resume.yaml             # Core info, skills, education, projects
│   │   ├── professional-experience.yaml  # Work history with version-specific achievements
│   │   └── cover-letter.yaml      # Cover letter templates
│   ├── mapping-rules/      # Transformation rule files
│   │   ├── resume.yaml             # Resume transformation rules (React-PDF schema)
│   │   ├── job_analysis.yaml      # Job posting analysis rules
│   │   └── cover_letter.yaml      # Cover letter transformation rules
│   └── tailor/             # Company-specific tailored outputs
│       └── [company-name]/ # Per-company folder structure
│           ├── resume.yaml         # Tailored resume
│           ├── job_analysis.yaml   # Job posting analysis
...
### Application Flow
1. **Data Generation**: `generate-data.ts` → merges `resume-data/sources/` files → `src/data/application.ts` TypeScript module
...
4. **Resume Tailoring**: Job posting analysis → `resume-data/tailor/[company]/` files → optimized PDF output
...
- `generate-data.ts`: YAML to TypeScript converter (auto-generates `src/data/application.ts`)
- `src/data/application.ts`: Auto-generated TypeScript data module from merged source files
- `src/pages/DocumentWrapper.tsx`: Placeholder PDF document component with Don Quixote content
- `src/pages/resume/index.tsx`: Complete professional resume document with source data transformation
- `src/index.ts`: Export barrel for modular component access
- `generate-pdf.ts`: Server-side rendering for file output
- `resume-data/sources/`: Source data files (resume.yaml, professional-experience.yaml, cover-letter.yaml)
- `resume-data/mapping-rules/`: Transformation rules for resume, job analysis, and cover letter generation
- `resume-data/tailor/`: Company-specific tailored outputs generated by resume-tailor agent
...
The project uses a modular data structure:

### Source Data (`resume-data/sources/`)
Comprehensive resume data split into focused files:
...
### Schema Transformation (`resume-data/mapping-rules/`)
Defines transformation rules to convert rich source data into various output formats:
...
The project includes example data files in `resume-data/sources/` for testing without personal information:
...
- **Data Ready**: Comprehensive resume data in `resume-data/sources/*.example.yaml` (228+ lines) with multi-version support
- **Schema Transformation**: `resume-data/mapping-rules/` provides React-PDF compatibility and other transformation rules
...
3. **Schema Generation**: Create optimized `resume-data/resume_tailored.yaml` following transformation rules  
4. **PDF Generation**: Generate tailored resume PDF with `bun run generate-pdf.ts -C company-name`

### Key Components

**Source Data (`resume-data/sources/`)**:
- Modular resume information split across focused files
- `resume.yaml`: Core info, skills, education, projects
- `professional-experience.yaml`: Work history with multi-version achievements
- `cover-letter.yaml`: Cover letter templates and content

**Tailored Output (`resume-data/tailor/[company-name]/`)**:
- Job-optimized files following transformation schema requirements
- `resume.yaml`: Prioritized technical expertise and curated achievements
- `job_analysis.yaml`: Structured job posting analysis and requirements
- `cover_letter.yaml`: Personalized cover letter with company research

**Transformation Rules (`resume-data/mapping-rules/`)**:
- `resume.yaml`: Schema compatibility requirements for React-PDF components
- `job_analysis.yaml`: Job posting analysis and extraction patterns
- `cover_letter.yaml`: Template transformation and personalization rules
...
# 2. Sub-agent generates resume-data/tailor/[company-name]/ files automatically
...
- **Data Generation Pipeline**: Source file merging → TypeScript conversion runs automatically before all commands
